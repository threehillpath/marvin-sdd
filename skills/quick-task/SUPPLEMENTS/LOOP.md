# TDD Implement Loop — Sub-agent Instructions

This document is assembled into the sub-agent prompt by `quick-task`. The sub-agent operates autonomously in a pre-created git worktree.

## 1. Setup

The worktree has been created by the orchestrator and is already on `<task-branch>`. All work must happen inside the worktree directory:
...
```bash
git -C <worktree-path> branch --show-current  # must print: <task-branch>
```
...
Read all source files relevant to this Task before writing any code. The orchestrator passed you a list of relevant file paths — read them yourself; do not assume their contents have been pre-read for you. Understand the existing structure fully before changing it.
...
**Requirement ambiguity** (the Task issue's requirement is underspecified or self-contradictory — a Task has no separate impl plan to conflict with, it is self-contained plus its source issue): Stop the loop. Describe the ambiguity specifically. Return to the caller for input.
...
git -C <worktree-path> commit -m "[TASK-XXXXX] <Title>"
```
...
Target `main`. A Task has no arch/impl hierarchy or intermediate branch to PR into — the Task branch merges straight to `main`.

Read `../../SHARED/PR_TEMPLATE.md` and use the **Task PR** template.
...
The PR body must include `Closes #<task-issue-number>`.

### Move Task issue to In Review

```bash
marvin board move <task-issue-number> in-review
```

### Return summary

Report back to the `quick-task` caller:
- PR URL
- PR number
...
This applies whether the correction was self-caught or raised in review. The Notes entries are read by `/quick-task`'s Mode B after merge.
