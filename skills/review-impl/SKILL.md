---
name: review-impl
description: Comprehensive code-review of the impl PR (<type>/PLAN-XXXXX/main → main) after it is opened by finish-impl
...
The JSON output includes `found`, `number`, `url`, and `state`. If `found` is `false`, stop: "No open impl PR found — run `/finish-impl $0` first." If `marvin` exits with code 2, surface to the user: "Configuration missing — run `/configure-plan-plugin` first." Capture the PR number. The head branch is `<type>/PLAN-XXXXX/main` (derived from the plan number parsed above); the base is `main` (also available as the `base` field in the `marvin pr find` result).

Confirm the local impl branch is current:

```bash
git fetch origin <type>/PLAN-XXXXX/main
```
...
>
> **PR to review**: #<pr-number> in repo `<repo>`. Head: `<type>/PLAN-XXXXX/main`. Base: `main`.
...
> git fetch origin main <type>/PLAN-XXXXX/main
> git log origin/main..origin/<type>/PLAN-XXXXX/main --oneline
> ```
>
...
```
Review of implementation <type>/PLAN-XXXXX/main (vs main)
...
- If `verdict` is `request-changes`: "Address findings on the impl branch (either as new commits to `<type>/PLAN-XXXXX/main` directly, or by opening a follow-up phase if the work is large), then re-run `/review-impl $0` for a fresh pass."
