---
name: unslop
description: Cut AI tells from any writing. Must always apply to prose.
argument-hint: (none)
...
- **Reserve opinions until asked.** When asked, react to facts instead of neutrally listing pros and cons.
...
13. **Em dash overuse.** Avoid em dashes entirely. Use periods or commas only (parentheses only for citations or when no other option exists, no en dashes, no hyphen-as-dash substitutes). Em dashes are an AI tell, and reaching for parentheses instead just trades one tell for another. If a thought needs separation, end the sentence or use a comma.
